...
let loaderAnimation = null; // Will hold the Lottie loading animation instance
...
function estimateUV(clouds, dt, sunrise, sunset) {
    ...
    const clearSky = (100 - cloudVal) / 100;
...
async function fetchWeather(city) {
    ...
    try {
        ...
        // Save precise coords to localStorage
...
async function fetchWeatherByCoords(lat, lon) {
    ...
    try {
        ...
        // Fetch outfit suggestion (Outfit Recommendation Fix!)
        fetchDashboardOutfit();
    } catch (err) {
        console.error('[Weather] Error:', err);
        showError(err.message || 'Failed to fetch weather data.');
...
async function fallbackIPLocation(lowErrorMsg) {
    try {
        console.log('[Geolocation] HTML5 Geolocation failed. Trying IP-based location fallback...');
        const res = await fetch('https://ipapi.co/json/');
        if (!res.ok) throw new Error('IP lookup status error');
        const data = await res.json();
        if (data.latitude && data.longitude) {
            console.log('[Geolocation] IP Fallback successful:', data.city);
            fetchWeatherByCoords(data.latitude, data.longitude);
            return;
        }
    } catch (e) {
        console.warn('[Geolocation] IP fallback failed:', e);
    }
    hideLoading();
    showError(lowErrorMsg || "Location unavailable.");
}
...
function handleCurrentLocation() {
    if (!navigator.geolocation) {
        ...
        showLoading();
        fallbackIPLocation("Geolocation is not supported by your browser.");
        return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
            console.warn("[Geolocation] High accuracy failed, trying low accuracy fallback...", error.message);
            navigator.geolocation.getCurrentPosition(
                (position) => {
                    const { latitude, longitude } = position.coords;
                    fetchWeatherByCoords(latitude, longitude);
                ...
                (lowError) => {
                    ...
                    if (lowError.code === lowError.POSITION_UNAVAILABLE) msg = "Location unavailable.";
                    if (lowError.code === lowError.TIMEOUT) msg = "Location request timed out.";
                    fallbackIPLocation(msg);
                },
                optionsLow
            );
        },
...
function renderDetails() {
    ...
    if (DOM.uvValue) DOM.uvValue.textContent = uv;
    if (DOM.uvLabel) DOM.uvLabel.textContent = uvLabel;
...
function getLocalOutfitRecommendation(tempCelsius, mainCondition) {
    ...
    if (tempCelsius < 5) { 
        top = "Heavy Coat"; bottom = "Thermals"; footwear = "Boots"; accessory = "Scarf"; 
    } else if (tempCelsius < 15) { 
        top = "Sweater"; bottom = "Jeans"; footwear = "Boots"; accessory = "Beanie"; 
    } else if (tempCelsius < 22) { 
        top = "Hoodie"; bottom = "Chinos"; footwear = "Sneakers"; 
    } else if (tempCelsius < 28) { 
        top = "Polo"; bottom = "Shorts"; footwear = "Canvas Shoes"; accessory = "Sunglasses"; 
    } else { 
        top = "Linen Shirt"; bottom = "Shorts"; footwear = "Sandals"; accessory = "Hat"; 
    }

    if (isRain) { 
        top += " + Raincoat"; accessory = "Umbrella"; 
    }

    return { top, bottom, footwear, accessory };
}

function renderOutfitUI(rec, container) {
    container.innerHTML = `
...
async function fetchDashboardOutfit() {
    ...
    // Render local quick suggestions immediately (Quick Get!)
    const currentTempC = currentWeatherData.main.temp - 273.15;
    const localRec = getLocalOutfitRecommendation(currentTempC, currentWeatherData.weather[0].main);
    renderOutfitUI(localRec, container);

    // Fetch AI insights asynchronously in background
    try {
        const forecastList = [];
        ...
        if (rec) {
            renderOutfitUI(rec, container);
        }
    } catch (err) {
        console.warn('[Dashboard Outfit AI] Error fetching AI suggestion, keeping local recommendation:', err);
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (searchInput) {
        ...
        const lastLon = localStorage.getItem('lastLon');
        if (lastLat && lastLon) {
            console.log('[Init] Auto-loading weather for coords:', lastLat, lastLon);
            fetchWeatherByCoords(lastLat, lastLon);
        } else {
