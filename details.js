...
function estimateUV(clouds, dt, sunrise, sunset) {
    ...
    const cloudVal = typeof clouds === 'number' ? clouds : 0;
    const prog = (dt - sunrise) / (sunset - sunrise);
    return Math.round(Math.sin(prog * Math.PI) * ((100 - cloudVal) / 100) * 11);
}
...
function getMoonIcon(phase) {
    if (phase < 0.03 || phase >= 0.97) return { icon: '🌑', text: 'New Moon' };
    if (phase < 0.22) return { icon: '🌒', text: 'Waxing Crescent' };
    if (phase < 0.28) return { icon: '🌓', text: 'First Quarter' };
    if (phase < 0.47) return { icon: '🌔', text: 'Waxing Gibbous' };
    if (phase < 0.53) return { icon: '🌕', text: 'Full Moon' };
    if (phase < 0.72) return { icon: '🌖', text: 'Waning Gibbous' };
    if (phase < 0.78) return { icon: '🌗', text: 'Last Quarter' };
...
function getPollenDetails(name, val) {
    if (!val || val <= 0) return { label: 'None', pct: 0, color: '#4ade80' };
    
    let low, mod, high;
    if (name === 'Grass') {
        low = 10; mod = 50; high = 200;
    } else if (name === 'Birch') {
        low = 30; mod = 150; high = 500;
    } else if (name === 'Alder') {
        low = 30; mod = 90; high = 250;
    } else if (name === 'Olive') {
        low = 15; mod = 75; high = 250;
    } else if (name === 'Ragweed') {
        low = 10; mod = 50; high = 250;
    } else {
        low = 10; mod = 50; high = 200; // Default
    }

    if (val <= low) return { label: 'Low', pct: 20, color: '#4ade80' };
    if (val <= mod) return { label: 'Moderate', pct: 50, color: '#facc15' };
    if (val <= high) return { label: 'High', pct: 80, color: '#f97316' };
    return { label: 'Extreme', pct: 100, color: '#ef4444' };
}
...
function renderExtra(extra) {
    ...
    const uvHourly = extra.hourly?.uv_index?.[hr] ?? null;
    const uvMax = extra.daily.uv_index_max?.[today] ?? null;
    const uvVal = uvHourly !== null ? uvHourly : uvMax;

    if (uvVal !== null) {
        const uvLabel = getUVLabel(uvVal);
        const uvColor = getUVColor(uvVal);
        set('uv-value', uvVal);
        set('uv-label', uvLabel);
        setCircularGauge($('uv-fill'), uvVal / 11, uvColor);
    }
...
function renderPollen(aqiData) {
    ...
    pollenEl.innerHTML = types.map(p => {
        const lvl = getPollenDetails(p.name, p.val);
...
async function loadDetailsPage() {
    ...
    try {
        ...
        if (lat && lon && lat !== 'undefined' && lon !== 'undefined' && lat !== 'null' && lon !== 'null') {
            const wRes = await fetch(`${BASE_URL}/weather?lat=${lat}&lon=${lon}&appid=${config.OWM_KEY}`);
        ...
        if (w.cod && parseInt(w.cod) !== 200) {
            throw new Error(w.message || 'Failed to fetch weather');
        }
        if (!w.coord) {
            throw new Error('Invalid coordinates in weather data');
        }

        const targetLat = w.coord.lat;
        const targetLon = w.coord.lon;
        renderCurrent(w);

        // Update Theme
        const tempCelsius = w.main.temp - 273.15;
        let theme = 'cold';
        if (tempCelsius < 0) theme = 'freezing';
        else if (tempCelsius >= 0 && tempCelsius < 15) theme = 'cold';
        else if (tempCelsius >= 15 && tempCelsius < 25) theme = 'mild';
        else if (tempCelsius >= 25 && tempCelsius < 35) theme = 'warm';
        else theme = 'hot';
        document.body.setAttribute('data-theme', theme);

        // Update stored city name & coordinates
        localStorage.setItem('lastCity', w.name);
        localStorage.setItem('lastLat', targetLat);
        localStorage.setItem('lastLon', targetLon);

        // Step 2: Parallel fetches
        const [fRes, extraRes, aqiOWMRes, aqiMeteoRes] = await Promise.allSettled([
            fetch(`${BASE_URL}/forecast?lat=${targetLat}&lon=${targetLon}&appid=${config.OWM_KEY}`),
            fetch(`${BASE_URL}/extra?latitude=${targetLat}&longitude=${targetLon}&daily=sunrise,sunset,moon_phase,uv_index_max,temperature_2m_max,temperature_2m_min,weathercode,precipitation_sum,precipitation_probability_max&hourly=visibility,cloudcover,uv_index&forecast_days=3&timezone=auto`),
            fetch(`${BASE_URL}/air_pollution?lat=${targetLat}&lon=${targetLon}&appid=${config.OWM_KEY}`),
            fetch(`${BASE_URL}/air-quality?latitude=${targetLat}&longitude=${targetLon}&hourly=pm2_5,grass_pollen,birch_pollen,alder_pollen,olive_pollen,ragweed_pollen&timezone=auto&forecast_days=3`)
        ]);
